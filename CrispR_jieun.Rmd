...
```{r Use_of_maker_of_count_matrix}
Counts <- Files_2_DF(Files,Index)
colnames(Counts) <- c("Contr_1","Contr_2","ToxA_1","ToxA_2","ToxB_1","ToxB_2")
head(Counts)
```
...
# decided to work in my directory
d <- wdf("KY_extdata/*gz")
Files <- Sys.glob(d)
basename(Files)
...
the library sgRNA has at least 30 reads in control, instead, the largest
number (instead of 30) that satisfies this condition is 5, too little for
a reasonable null model.  This tells that a good quality control has at least
6 million reads, although if the counts is a bit smaller, we can reduce the
threshold from 30 to 25 or 20.  So we try another data set
```{r}
fn <- file.path(
system.file("extdata", package = "CRISPRcleanR"), "HT-29_counts.tsv"
)
ccr.AnalysisPipeline(
  file_counts = fn,
